...
def update_analog_sensor_plot_for_user(
    ...
    # Save to file
    fig = ax.get_figure()
    fig.savefig(r'./output/analog_plot.png', dpi=300, bbox_inches='tight');
    plt.close(fig);
...
def update_digital_sensor_plot_for_user(
    ...
    # Save to file
    fig = ax.get_figure()
    fig.savefig(r'./output/digital_plot.png', dpi=300, bbox_inches='tight');
    plt.close(fig);
...
def train_prophet_model(sensor: str, df: pd.DataFrame) -> None:
    ...
    dump(model, f'03-models/distributed_prophet_{sensor}.joblib')
